...
class Order extends BaseComponent{
    ...
    async postOrder(req, res, next){
        ...
        form.parse(req, async (err, field, files)=>{
            ...
            let order_id;
            try{
                cartDetail = await CartModel.findOne({id: cart_id});//购物车详情
                order_id = await this.getId('order_id'); //订单id
            }catch(err){
            ...
            const deliver_feee = {price: cartDetail.cart.deliver_amount}
            const orderObj = {
                basket: {
                    group: entities,
                    packing_fee: {
    ...
    async getDetail(req, res, next){
        const {user_id, order_id } = req.param;
        try{
            if (!user_id || !Number(user_id)) {
                throw new Error('user_id参数错误')
        ...
        try{
            const order = await OrderModel.findOne({id:order_id},'-id');
    ...
    async getOrdersContent(req, res, next){
